...
async function filterData(type, element) {
    ...
    filtrirano = applyAdvancedFiltersToItems(filtrirano);
...
function getFilteredByType(type) {
    const user = JSON.parse(localStorage.getItem('user'));
    if (type === 'my') {
        return all_data.filter(item => user && item.korisnikId === user.id);
    }
...
function applyAdvancedFiltersToItems(items) {
    let filtered = items;

    if (currentSubjectFilter) {
        filtered = filtered.filter(item =>
            String(item.predmet || '').toLowerCase() === currentSubjectFilter.toLowerCase()
        );
    }

    if (currentProfessorFilter) {
        filtered = filtered.filter(item =>
            String(item.profesor || '').toLowerCase().includes(currentProfessorFilter)
        );
    }
...
function applyAdvancedFilters() {
    const select = document.getElementById('subjectFilterSelect');
    const professorInput = document.getElementById('professorFilterInput');

    currentSubjectFilter = select ? select.value.trim() : '';
    currentProfessorFilter = professorInput ? professorInput.value.trim().toLowerCase() : '';

    let filtered = getFilteredByType(currentFilterType);
    filtered = applyAdvancedFiltersToItems(filtered);
    osveziPrikaz({ gradiva: filtered });
}
